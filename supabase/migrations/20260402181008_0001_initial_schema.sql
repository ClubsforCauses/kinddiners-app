/*
  # Initial Schema for Kind Diners Platform v2
  ...
  2. Core Tables
    - membership_plans: plan catalog (CLASSIC, AMBASSADOR, etc.)
    - restaurants: partner restaurants with full details
    - profiles: unified identity (members, restaurant_admins, admins)
    - restaurant_admins: restaurant portal access
    - memberships: subscriptions per profile
    - referral_codes: unique codes for sponsors
    - referrals: attribution tracking
    - promo_codes: promotional codes
    - gift_orders: gift purchases
    - gift_codes: redeemable gift codes
    - redemptions: member visits to restaurants
    - payouts: referral payout batches
    - payout_items: individual referral earnings
    - tremendous_records: Tremendous API integration
    - plan_payout_rates: configurable payout amounts
...
CREATE TYPE "public"."app_role" AS ENUM('member', 'restaurant_admin', 'admin');
CREATE TYPE "public"."email_event_type" AS ENUM('welcome', 'cancel', 'renewal', 'gift_delivery', 'password_reset', 'referral', 'other');
CREATE TYPE "public"."gift_code_status" AS ENUM('available', 'redeemed', 'expired', 'revoked');
CREATE TYPE "public"."itb_application_status" AS ENUM('pending', 'approved', 'rejected');
CREATE TYPE "public"."membership_source" AS ENUM('direct', 'promo', 'gift', 'complimentary', 'itb', 'first_responder');
CREATE TYPE "public"."membership_status" AS ENUM('active', 'canceled', 'past_due', 'trialing', 'gifted', 'expired', 'inactive');
CREATE TYPE "public"."payout_status" AS ENUM('draft', 'pending_submission', 'pending_approval', 'approved', 'paid', 'rejected', 'canceled');
CREATE TYPE "public"."redemption_method" AS ENUM('member_scan', 'restaurant_scan');
CREATE TYPE "public"."tremendous_order_status" AS ENUM('pending', 'paid', 'rejected', 'canceled');
...
CREATE INDEX IF NOT EXISTS "audit_logs_actor_profile_id_idx" ON "audit_logs" USING btree ("actor_profile_id");
...
CREATE INDEX IF NOT EXISTS "email_events_recipient_email_idx" ON "email_events" USING btree ("recipient_email");
CREATE INDEX IF NOT EXISTS "email_events_sent_at_idx" ON "email_events" USING btree ("sent_at");
CREATE INDEX IF NOT EXISTS "gift_codes_gift_order_id_idx" ON "gift_codes" USING btree ("gift_order_id");
CREATE INDEX IF NOT EXISTS "gift_codes_redeemed_profile_id_idx" ON "gift_codes" USING btree ("redeemed_profile_id");
CREATE INDEX IF NOT EXISTS "memberships_profile_id_idx" ON "memberships" USING btree ("profile_id");
CREATE INDEX IF NOT EXISTS "memberships_plan_id_idx" ON "memberships" USING btree ("plan_id");
CREATE INDEX IF NOT EXISTS "memberships_status_idx" ON "memberships" USING btree ("status");
CREATE INDEX IF NOT EXISTS "memberships_billing_subscription_id_idx" ON "memberships" USING btree ("billing_subscription_id");
CREATE UNIQUE INDEX IF NOT EXISTS "plan_payout_rates_plan_code_recipient_type_idx" ON "plan_payout_rates" USING btree ("plan_code","recipient_type");
...
CREATE INDEX IF NOT EXISTS "profiles_restaurant_id_idx" ON "profiles" USING btree ("restaurant_id");
CREATE INDEX IF NOT EXISTS "profiles_billing_customer_id_idx" ON "profiles" USING btree ("billing_customer_id");
CREATE INDEX IF NOT EXISTS "profiles_billing_subscription_id_idx" ON "profiles" USING btree ("billing_subscription_id");
CREATE INDEX IF NOT EXISTS "profiles_referral_code_id_idx" ON "profiles" USING btree ("referral_code_id");
CREATE INDEX IF NOT EXISTS "redemptions_profile_id_idx" ON "redemptions" USING btree ("profile_id");
CREATE INDEX IF NOT EXISTS "redemptions_restaurant_id_idx" ON "redemptions" USING btree ("restaurant_id");
CREATE INDEX IF NOT EXISTS "redemptions_restaurant_admin_id_idx" ON "redemptions" USING btree ("restaurant_admin_id");
CREATE INDEX IF NOT EXISTS "redemptions_redeemed_at_idx" ON "redemptions" USING btree ("redeemed_at");
CREATE INDEX IF NOT EXISTS "referrals_referred_profile_id_idx" ON "referrals" USING btree ("referred_profile_id");
CREATE INDEX IF NOT EXISTS "referrals_referral_code_id_idx" ON "referrals" USING btree ("referral_code_id");
CREATE INDEX IF NOT EXISTS "restaurants_active_idx" ON "restaurants" USING btree ("active");
